...
class Activities(commands.Cog):
    ...
    async def dig(self, ctx):
        inventory = await edb.get_user_inventory(ctx, user_id=ctx.author.id)
        if 'shovel' not in inventory:
            ...
            current_bal = await edb.get_user_balance(user_id=ctx.author.id)
    ...
    async def hunt(self, ctx):
        ...
        if 'axe' or 'rifle'not in inventory:
            ...
            if 'axe' in inventory:
                ...
                await edb.update_balance(user_id=ctx.author.id, wallet=coins)
